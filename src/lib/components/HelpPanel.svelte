...
{#if open}
  ...
  ></div>
  <div
    class="fixed left-1/2 top-1/2 z-[301] flex max-h-[min(80vh,640px)] w-[min(92vw,520px)] -translate-x-1/2 -translate-y-1/2 flex-col overflow-hidden rounded-2xl border border-white/10 bg-[#1a1e26] shadow-2xl"
    role="dialog"
      ...
      <dl class="space-y-4">
        {#each GLOSSARY as entry}
          <div class="rounded-xl border border-white/8 bg-white/[0.02] px-4 py-3">
            <dt class="text-sm font-semibold text-indigo-200/95">{entry.term}</dt>
            <dd class="mt-1.5 text-xs leading-relaxed text-white/55">{entry.description}</dd>
          </div>
        {/each}
      </dl>
    </div>
