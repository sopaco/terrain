...
**模块路径**：`crates/mind-mesh-core/src/assets/`
**生成日期**：2026-06-14
**分析置信度**：8/10
...
知识资产管理是 MindMesh 的"数据中台"——它不直接存储数据，而是协调"什么资产该在什么时候生成、放在哪里"。具体来说，它管理四条资产生产线：（1）Agent 友好的架构上下文（`agent/context.md`），（2）AI 工程环境集成（Skills/工具链/AGENTS.md），（3）Litho 和 SDD 的工作计划和提示构建，（4）repomix 源码打包。
...
3. **Litho 计划构建**——`litho.rs` 中的 `plan_litho_generation()` 构建 `LithoPlan`（`crates/mind-mesh-core/src/schema.rs:162`），包含技能路径、输出目录、工作区目录。`build_litho_generation_prompt()` 和 `build_litho_composition_prompt()` 分别生成全流水线和编排阶段的 ACP Agent 提示。
...
| 组件/类型 | 文件路径 | 核心职责 |
|---------|---------|---------|
| `LithoPlan` | `crates/mind-mesh-core/src/schema.rs:162` | Litho 生成计划（skill_dir, human_output_dir, litho_workspace_dir） |
| `ContextOverview` | `crates/mind-mesh-core/src/assets/context_layers.rs` | 上下文概览摘要 |
| `AssetGenerationPlan` | `crates/mind-mesh-core/src/assets/mod.rs:55` | 完整资产生成计划 |
...
4. 写入：将输出写入 `.mind-mesh/` 相应目录

---

## 关键接口与扩展点

- **扩展点**：Skill 目录是可配置的（通过 `MIND_MESH_LITHO_SKILL` 环境变量），用户可以自定义 Litho 的文档生成逻辑
- **可插拔设计**：`resolve_litho_skill_dir()`（`crates/mind-mesh-core/src/assets/litho.rs:17`）优先使用环境变量指定的 skill，其次使用默认内嵌 skill
