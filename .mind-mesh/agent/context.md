...
| Meso | `agent/context.md` §4–7、`human/` | 按需 `read-context --section` |
...
| chat | DeepWiki 问答：流式输出、工具调用、引用提取 | `crates/mind-mesh-agent/src/chat.rs` |
...
| acp | OpenCode ACP 配置、环境变量注入、可用性检测 | `crates/mind-mesh-agent/src/acp.rs` |
| mind-mesh-cli | clap 入口：list/scan/search/tools/assets/env | `crates/mind-mesh-cli/src/main.rs` |
| 桌面 UI | Tauri IPC 命令 + Svelte 面板（Ask/Litho/SDD/Env） | `src-tauri/src/commands.rs`、`src/` |
...
3. Native LLM 或 ACP Agent 执行，可调用 `grep-pack`/`read-context`/`search`
...
- **语言/运行时**：Rust（workspace：`mind-mesh-core`、`mind-mesh-agent`、`mind-mesh-cli`）、Tokio 异步
- **桌面壳**：Tauri v2（`src-tauri/`），IPC 暴露 ~30 命令
...
- **ACP**：OpenCode 兼容代理（`MIND_MESH_ACP_*` 配置）
- **源码打包**：repomix-core（`architecture-context` 策略）
...
| 文档引擎 | `crates/mind-mesh-core/src/doc.rs` | YAML frontmatter 解析 |
| CLI tools 子命令 | `crates/mind-mesh-cli/src/main.rs` | `grep-pack`、`read-pack-file`、`read-context` |
| Tauri IPC | `src-tauri/src/commands.rs` | chat/litho/sdd/scan 事件流 |
...
| Agent arch skill | `preset_skills/agent-architecture-skill/SKILL.md` | 本文件生成契约 |
| Env catalog | `env-catalog/catalog.json` | Skills/AGENTS.md 集成清单 |
| 捆绑工具部署 | `crates/mind-mesh-core/src/agent_tools_deploy.rs` | codegraph/rtk 二进制 |