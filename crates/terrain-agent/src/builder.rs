...
- agent/meta-inputs.md — developer meta collected from terrain-meta.json (module hints, ADRs, glossaries)
...
pub fn build_agent(model: Arc<dyn Llm>, config: AgentConfig) -> Result<Arc<dyn Agent>> {
    ...
    let agent = builder.build().context("failed to build Terrain agent")?;
    Ok(Arc::new(agent))
}

pub fn knowledge_paths_from_env() -> KnowledgePaths {
    if let Ok(repo) = std::env::var("TERRAIN_REPO_PATH") {
        return KnowledgePaths::with_workspace_repo(repo);
    }
    if let Ok(knowledge_root) = std::env::var("TERRAIN_KNOWLEDGE_ROOT") {
        let path = Path::new(&knowledge_root);
        if path
            .file_name()
            .and_then(|name| name.to_str())
            .is_some_and(|name| name == ".terrain")
