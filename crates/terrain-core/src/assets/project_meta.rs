//! Developer-defined project metadata (`terrain-meta.json`) for Agent context generation.
//!
//! Repositories may place one or more `terrain-meta.json` files. Before generating
//! `agent/context.md`, Terrain collects referenced files and injects them into the LLM prompt.
...
pub const META_FILENAME: &str = "terrain-meta.json";
...
pub fn has_repo_meta_configured(repo: &Path) -> bool {
    repo.join(META_FILENAME).is_file() || repo.join(".terrain").join(META_FILENAME).is_file()
}

/// Discover `terrain-meta.json` under the repository (root, `.terrain/`, gitignore-aware walk).
pub fn discover_meta_files(repo: &Path) -> Vec<PathBuf> {
    let mut found = Vec::new();

    for candidate in [
        repo.join(META_FILENAME),
        repo.join(".terrain").join(META_FILENAME),
    ] {
...
/// Auto-scan `.terrain/knowledge/**/*.md` for private domain knowledge.
