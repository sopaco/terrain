...
const BEGIN_RE: &str = r"<!-- terrain:begin (\S+) v(\d+) -->";
...
pub fn agents_md_ready(repo: &Path) -> bool {
    ...
    content.contains("<!-- terrain:begin knowledge-guide")
}
...
fn replace_or_append_block(content: &str, id: &str, _version: u32, wrapped: &str) -> String {
    let begin_re = Regex::new(&format!(
        r"<!-- terrain:begin {id} v\d+ -->[\s\S]*?<!-- terrain:end {id} -->"
    ))
    .expect("valid regex");
...
mod tests {
    ...
    fn replaces_existing_block() {
        let content = "intro\n\n<!-- terrain:begin foo v1 -->\nold\n<!-- terrain:end foo -->\n";
        let wrapped = "<!-- terrain:begin foo v2 -->\nnew\n<!-- terrain:end foo -->";
