...
/// Discover macOS arm64 prebuilts from `packages/{rtk,terrain,codegraph}/darwin-arm64/`.
pub fn discover_bundled_tools_from_packages() -> BundledTools {
    ...
    BundledTools {
        ...
        terrain_cli: executable_if_exists(
            root.join("terrain").join(PLATFORM_DIR).join("terrain"),
        ),
...
pub fn resolve_sidecar_next_to_exe(exe_dir: &Path, name: &str) -> Option<PathBuf> {
    let triple = option_env!("TERRAIN_TARGET_TRIPLE").unwrap_or("aarch64-apple-darwin");
    let candidates = [
