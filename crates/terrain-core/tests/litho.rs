...
fn count_human_docs_only_counts_human_section() {
    ...
    fs::create_dir_all(terrain.join("human")).unwrap();
    fs::create_dir_all(terrain.join("agent")).unwrap();
    fs::create_dir_all(terrain.join("interfaces")).unwrap();
    fs::write(terrain.join("human/1.概述.md"), "# overview\n").unwrap();
    fs::write(terrain.join("agent/context.md"), "# ctx\n").unwrap();
    fs::write(terrain.join("interfaces/ping.md"), "# ping\n").unwrap();

    let _lock = terrain_core::registry::registry_test_lock();
    let registry_dir = tempfile::tempdir().unwrap();
    let registry_file = registry_dir.path().join("registry.json");
    unsafe {
        std::env::set_var("TERRAIN_REGISTRY_FILE", &registry_file);
    }
    terrain_core::register_project("demo", &root.display().to_string()).unwrap();
