//! Deploy MindMesh-bundled CLIs where external Coding Agents can invoke them.
//!
//! App bundle / `packages/` paths are not on Agent PATH — we symlink into
//! `~/.mind-mesh/bin/` and `~/.mind-mesh/tools/`. Re-deploy only when missing,
//! broken, or forced.
...
use crate::bundled_tools::{bundled_mind_mesh_cli, bundled_rtk, ensure_bundled_tools_initialized};
...
pub struct AgentToolPaths {
    ...
    pub mind_mesh: Option<String>,
    pub codegraph_runtime: Option<String>,
}

pub fn agent_bin_dir() -> PathBuf {
    user_home()
        .map(|h| h.join(".mind-mesh/bin"))
        .unwrap_or_else(|| PathBuf::from(".mind-mesh/bin"))
}

pub fn agent_tools_runtime_dir() -> PathBuf {
    user_home()
        .map(|h| h.join(".mind-mesh/tools"))
        .unwrap_or_else(|| PathBuf::from(".mind-mesh/tools"))
}
...
pub fn deploy_agent_toolchain_with_options(opts: DeployOptions) -> Result<AgentToolPaths> {
    ...
    if let Some(src) = bundled_mind_mesh_cli() {
        let dest = bin_dir.join("mind-mesh");
        symlink_ensure(&dest, &src, opts.force)?;
        paths.mind_mesh = Some(dest.display().to_string());
    }
    ...
    if paths.rtk.is_none() && paths.codegraph.is_none() && paths.mind_mesh.is_none() {
        return Err(CoreError::InvalidDoc(
            "无可用内置工具可部署到 ~/.mind-mesh/bin/".into(),
        ));
    }
...
pub fn write_repo_agent_tools_manifest(repo: &Path, paths: &AgentToolPaths) -> Result<String> {
    ...
    let doc = serde_json::json!({
        ...
        "mind_mesh": paths.mind_mesh,
        "codegraph_runtime": paths.codegraph_runtime,
        "usage": {
            "rtk": "Prefer absolute path from `rtk` field, or ensure ~/.mind-mesh/bin is on PATH",
            "codegraph": "Prefer absolute path from `codegraph` field; index is per-repo under .codegraph/",
            "mind_mesh": "ACP / CLI knowledge tools: `mind-mesh tools …`"
        }
    });
...
mod tests {
    ...
    fn agent_bin_dir_under_home() {
        if user_home().is_some() {
            assert!(agent_bin_dir().ends_with(".mind-mesh/bin"));
        }
    }
}
