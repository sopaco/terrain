...
/// Walk repository entries for `mind-mesh-meta.json` discovery, honoring `.gitignore`.
...
fn allow_meta_discover_entry(path: &Path) -> bool {
    !is_under_git_dir(path) && !is_mind_mesh_knowledge(path)
}
...
pub fn is_path_gitignored(repo: &Path, path: &Path) -> bool {
    if is_mind_mesh_knowledge(path) {
        return true;
    }
...
pub fn is_mind_mesh_knowledge(path: &Path) -> bool {
    ...
    parts
        .windows(2)
        .any(|w| w[0] == ".mind-mesh" && w[1] == "knowledge")
}
...
mod tests {
    ...
    fn gitignore_skips_target_during_meta_discover() {
        ...
        fs::write(
            repo.join("target/deep/mind-mesh-meta.json"),
            r#"{"version":1,"inputs":[]}"#,
        )
        .unwrap();
        fs::write(
            repo.join("mind-mesh-meta.json"),
            r#"{"version":1,"inputs":[]}"#,
        )
        .unwrap();
    ...
    const META_FILENAME: &str = "mind-mesh-meta.json";
