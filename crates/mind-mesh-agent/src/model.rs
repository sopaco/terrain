...
/// `OPENAI_API_KEY` or `MIND_MESH_API_KEY`.
fn read_api_key() -> Option<String> {
    std::env::var("OPENAI_API_KEY")
        .ok()
        .or_else(|| std::env::var("MIND_MESH_API_KEY").ok())
        .filter(|k| !k.is_empty())
}

/// Load effective LLM config.
///
/// When `~/.mind-mesh/settings.json` exists, UI-saved provider/profile values win.
...
fn apply_env_overrides(config: &mut ModelConfig) {
    if let Ok(p) = std::env::var("MIND_MESH_LLM_PROVIDER") {
        config.provider = parse_provider(&p);
    }
    if let Ok(m) = std::env::var("MIND_MESH_MODEL") {
        config.model = m;
    }
...
pub fn build_llm(config: &ModelConfig) -> Result<Arc<dyn Llm>> {
    let inner = match config.provider {
        LlmProvider::Ollama => {
            ...
            if let Ok(ctx) = std::env::var("MIND_MESH_OLLAMA_CTX") {
...
fn build_openai_compatible(config: &ModelConfig) -> Result<Arc<dyn Llm>> {
    let api_key = config
        ...
        .clone()
        .filter(|k| !k.is_empty())
        .context("OPENAI_API_KEY or MIND_MESH_API_KEY is required")?;
    let base_url = config
        .openai_base_url
        .clone()
        .filter(|u| !u.is_empty())
        ...
        LlmProvider::Openai => {
            ...
            (
                ...
                if ok {
                    ...
                    "Set OPENAI_API_KEY or MIND_MESH_API_KEY".into()
                },
            )
        }
...
mod tests {
    ...
    fn settings_file_wins_over_env_provider_and_base_url() {
        ...
        unsafe {
            ...
            std::env::set_var("MIND_MESH_LLM_PROVIDER", "openai");
            std::env::set_var("OPENAI_BASE_URL", "https://integrate.api.nvidia.com/v1");
            std::env::set_var("MIND_MESH_MODEL", "stepfun-ai/step-3.7-flash");
        }
        ...
        unsafe {
            std::env::remove_var("MIND_MESH_LLM_PROVIDER");
            std::env::remove_var("OPENAI_BASE_URL");
            std::env::remove_var("MIND_MESH_MODEL");
        }
    }
}
