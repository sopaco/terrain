...
impl InvokeError {
  ...
  /// Convert a serializable value into an invoke error without a blanket `From` impl.
  #[inline]
  pub fn from_serializable<T: Serialize>(value: T) -> Self {
    serde_json::to_value(value)
      .map(Self)
      .unwrap_or_else(|e| Self::from_error(e))
  }
}
...
impl<T: IpcResponse, E: Serialize> From<Result<T, E>> for InvokeResponse {
  #[inline]
  fn from(result: Result<T, E>) -> Self {
    match result {
      ...
      Err(err) => Self::Err(InvokeError::from_serializable(err)),
    }
  }
}
...
impl<R: Runtime> InvokeResolver<R> {
  ...
  pub fn reject<T: Serialize>(self, value: T) {
    Self::return_result(
      self.webview,
      self.responder,
      InvokeResponse::Err(InvokeError::from_serializable(value)),
      self.cmd,
      self.callback,
      self.error,
    )
