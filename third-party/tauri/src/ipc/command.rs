...
pub mod private {
  ...
  impl<T: IpcResponse, E: Serialize> ResultKind for Result<T, E> {}
  ...
  impl ResultTag {
    ...
    where
      ...
      E: Serialize,
    {
      resolver.respond(value.map_err(InvokeError::from_serializable))
    }
    ...
    {
      Ok(value
        .map_err(InvokeError::from_serializable)?
        .body()?)
    }
  }
  ...
  impl<T: IpcResponse, E: Serialize, F: Future<Output = Result<T, E>>> ResultFutureKind for F {}
  ...
  impl ResultFutureTag {
    ...
    {
      let response = value.await.map_err(InvokeError::from_serializable)?;
      Ok(response.body()?)
    }
  }
}
